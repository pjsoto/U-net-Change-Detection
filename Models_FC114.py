...
class Models():
    def __init__(self, args, dataset):
        ...
        if self.args.phase == 'train':
            ...
            # Avoiding the past deforested pixels by using the mask_c
            self.masked_loss = tf.reduce_sum(self.mask_c * temp)
            self.sum_mask = tf.reduce_sum(self.mask_c)
    ...
    def Train(self):
        # In this training function, the core idea lies in the usage of patch coordinates instead of the traditional patches.
        # The dataset variable contains the original images, as well as the coordinates of training/validation patches.
        best_f1score = 0
        pat = 0
        # As we have an imbalance problem, weighted loss has been used to alleviate such issue. In the following are defined the weights for each class.  
        class_weights = []
        ...
        # Defining the data augmentation procedures that will be applied during the training
        ...
        while (e < self.args.epochs):
            ...
            for b in batchs:
                #Extracting the batches coordinates and domain indexes to choose the correspondent image with the coordinates 
                ...
                # Extracting the transformation indexes 
                if self.args.data_augmentation:
                ...
                # taking the available training pixels aiming  at computing the metrics over such pixels
...
def Metrics_For_Test_M(heat_map,
    ...
    sio.savemat(save_path + 'hit_map.mat' , {'hit_map': heat_map})
    Probs_init = heat_map
    positive_map_init = np.zeros_like(Probs_init)
